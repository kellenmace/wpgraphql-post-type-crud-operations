...
class UpdateProject implements Hookable {
    use UpsertProject;
    ...
    public function register_input_fields(): void  {
        $input_type = 'Update' . ProjectPostType::GRAPHQL_SINGLE_NAME . 'Input';
    ...
    public function validate( $source, array $args, AppContext $context, ResolveInfo $info, $field_resolver, string $type_name, string $field_key ): void {
        if ( 'RootMutation' !== $type_name || ! $this->is_update_project_mutation( $field_key ) ) {
            return;
        }
    ...
    public function save_additional_data( int $post_id, array $input, WP_Post_Type $post_type_object, string $mutation_name ): void {
        if ( ! $this->is_update_project_mutation( $mutation_name ) ) {
            return;
        }
    ...
    private function is_update_project_mutation( string $mutation_name ): bool {
        return 'update' . ProjectPostType::GRAPHQL_SINGLE_NAME === $mutation_name;
    }
}
